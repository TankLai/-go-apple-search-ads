...
// https://developer.apple.com/documentation/apple_search_ads/campaign?changes=latest_major
type Campaign struct {
	...
	AdChannelType                      AdChannelType      `json:"adChannelType,omitempty"`
	BillingEvent                       BillingEvent       `json:"billingEvent,omitempty"`
	SupplySources                      []SupplySources    `json:"supplySources,omitempty"`
}
...
func (s *CampaignService) Edit(ctx context.Context, id int64, data *Campaign, clearGeoTargetingOnCountryOrRegionChange bool) (*Campaign, *Response, error) {
	...
	if clearGeoTargetingOnCountryOrRegionChange {
		putData["clearGeoTargetingOnCountryOrRegionChange"] = clearGeoTargetingOnCountryOrRegionChange
	}
	u := fmt.Sprintf("campaigns/%d", id)
...
func (s *CampaignService) Find(ctx context.Context, sel *Selector) ([]*Campaign, *Response, error) {
	req, err := s.client.NewRequest("POST", "campaigns/find", sel)
	if err != nil {
		return nil, nil, err
	}
