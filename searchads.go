...
func NewV4Client(httpClient *http.Client, Authorization string, orgID *int64) (*Client, error) {
	if httpClient == nil {
		httpClient = &http.Client{}
	}
	baseURL, _ := url.Parse(defaultBaseURL)
	c := &Client{client: httpClient, BaseURL: baseURL, UserAgent: userAgent, OrgID: orgID, Authorization: Authorization}
	c.common.client = c
	c.Campaign = (*CampaignService)(&c.common)
	c.CampaignNegativeKeyword = (*CampaignNegativeKeywordService)(&c.common)
	c.AdGroup = (*AdGroupService)(&c.common)
...
func (c *Client) NewRequest(method, urlStr string, body interface{}) (*http.Request, error) {
	...
	if c.Authorization != "" {
		...
		if c.OrgID != nil {
			req.Header.Set("X-AP-Context", fmt.Sprintf("orgId=%v", *c.OrgID))
		}
	} else if c.OrgID != nil {
		// for v2
